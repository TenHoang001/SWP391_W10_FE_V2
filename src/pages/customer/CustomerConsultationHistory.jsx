...
import { Search, XCircle, Clock, CheckCircle2, AlertCircle } from 'lucide-react';
...
const CustomerConsultationHistory = () => {
  ...
  const filteredConsultations = consultations
    .filter(consultation => 
      consultation.assignedDoctor?.fullName.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .filter(consultation => 
            ...
            return (
              <div key={consultation.requestId} className='flex justify-between rounded bg-white p-3 shadow'>
                <div className='flex items-center gap-4'>
                  ...
                  />
                  <div>
                    <p className='font-semibold'>BS: {consultation.assignedDoctor?.fullName || 'Chưa phân công'}</p>
                    <p className='text-sm text-gray-600'>Trẻ: {consultation.child?.fullName}</p>
                  </div>
                </div>
                <div className='flex items-center gap-4'>
                  <div>
                  ...
                  <Link to={`/customer/consultationChat/${consultation.requestId}`}>
                    <button className={`flex w-40 items-center gap-2 rounded-full ${statusInfo.color} px-3 py-1`}>
                      {statusInfo.icon}
                      {statusInfo.text}
                    </button>
                  </Link>
